...
function updateBatteryStatus(battery) {
    ...
    // STEP 3e: Create or update the image using battery level
    const batteryLevel = Math.round(battery.level * 100);
    let roboImg = document.getElementById("roboImage");

    if (!roboImg) {
    roboImg = document.createElement("img");
    roboImg.id = "roboImage";
    document.body.appendChild(roboImg);
    }
    roboImg.src = `https://robohash.org/${batteryLevel}.png`;
    roboImg.alt = `Robot for battery level ${batteryLevel}%`;
}
